...
openssl ca -config $SSL_DIR/openssl.cnf -revoke $SSL_CERTS_DIR/${CERT_SN}.pem -passin pass:ca-crypto
if [ "$?" != "0" ]; then
...
openssl ca -config $SSL_DIR/openssl.cnf -gencrl -out $SSL_DIR/crl/crl.pem -passin pass:ca-crypto
if [ "$?" != "0" ]; then
